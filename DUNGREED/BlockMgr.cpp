...
void CBlockMgr::Initialize()
{
	...
	CBmpMgr::Get_Instance()->Insert_Bmp(L"../Image2/Map/SecondFloor_2.bmp", L"Tile01");
...
void CBlockMgr::Render(HDC hDC)
{
	...
	case BLOCK1:
		...
		hMemDC = CBmpMgr::Get_Instance()->Find_Image(L"Tile01");
...
void CBlockMgr::Load_Block()
{
	...
	while (true)
	{
		...
		case BLOCK1:
			//Info값 대로 블럭 생성
			pNewObj = CAbstractFactory<CBlock_01>::Create_Block(OBJ_BLOCK, Block.Get_Info());
			//해당 리스트에 오브젝트 추가
...
CObj* CBlockMgr::Create_Block(INFO* _tInfo)
{
	...
	{
	case BLOCK1:
		pObj = CAbstractFactory<CBlock_01>::Create_Block(OBJ_BLOCK, _tInfo);
		break;
	/*case BLOCK_FIRE:
